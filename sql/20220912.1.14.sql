...
alter table eml_delivery add message_id varchar(500) null;

-- create a id_conversation for a delivery when it receives the first response.
alter table eml_delivery add id_conversation uuid null;

-- record responses from leads in the delivery table
alter table eml_delivery add is_response boolean not null default false;
